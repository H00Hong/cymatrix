...
class interp1d:
    ...
    def __init__(self,
                 x: Union[Array, Matrix],
                 y: Union[Array, Matrix],
                 kind: Literal[1, 2, 3] = 3,
                 boundary: Literal['natural', 'periodic',
    ...
    def __call__(self, x: Union[Array, Matrix]) -> Matrix:
        ...
        if x.ndim == 0:
            ...
            x_in = x
        if x_in.ndim != 1:
            raise ValueError('x_in.ndim must == 1')
        ...
        if self.y.ndim == 1:
            ...
            res = [
                concat([coe[val] @ X[ii]
                        for ii, val in enumerate(x_in_ind)], 1).vector()
...
def polyfit(x: Union[Array, Matrix], y: Union[Array, Matrix],
            n: int) -> Matrix:
    #
...
def _filter2(x: Matrix,
             length: int = 3,
             axis: int = 0,
             mod: Callable = mean) -> Matrix:
    if length % 2 != 1:
...
def savgol_filter(x: Matrix,
    ...
    if not isinstance(poly, int):
        raise TypeError('poly must be int')
    if poly <= 0 or poly > length:
        raise ValueError('poly must > 0 and poly <= length')
    if length % 2 != 1:
        raise ValueError('length must be odd')
    _n = length // 2
...
def _ddfdxx(f: Callable, x_in: Matrix, i1: int, i2: int, *args,
            **kwargs) -> Matrix:
    """近似二阶偏导"""
    dx1 = zeros(x_in.size)
    dx2 = zeros(x_in.size)
...
def bisection(
    ...
    y1, y2 = func(bounds[0], *args, **kwargs), func(bounds[1], *args, **kwargs)
    if y1 * y2 > 0:
        raise ValueError('there are no zeros in the bounds')
    if y1 < y2:
        k1, k2 = bounds
    else:
        k2, k1 = bounds
...
if __name__ == '__main__':
    ...
    res = {
        'x':
        Matrix([1.17479537e+02, 6.15118989, -4.72717596e-02]),
        'se':
        Matrix([
        ...
        3,
        'grad':
        Matrix([2.45430354e-08, 9.92523326e-08, -2.19310754e-06]),
        'optimality':
        2.193107544457007e-06
    }
